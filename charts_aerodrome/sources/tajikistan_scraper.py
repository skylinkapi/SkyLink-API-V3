"""
Tajikistan CAICA Scraper
Fetches aerodrome charts from Tajikistan's AIP via CAICA system.
URL: https://www.caica.ru/aiptjk/validaip/html/menueng.htm

Airports: UTDD (Dushanbe), UTDL (Khujand), UTDK (Kulob), UTDT (Bokhtar)
"""
...
def get_aerodrome_charts(icao_code: str, verbose: bool = False) -> List[Dict[str, str]]:
    """
    Fetch aerodrome charts for a Tajikistan airport.
    
    Args:
        icao_code: ICAO code (UTDD, UTDL, UTDK, UTDT)
    ...
    menu_url = f"{base_url}/aiptjk/validaip/html/menueng.htm"
    ...
    # Structure: ItemBegin("id", "","ICAO. NAME"); followed by ItemLink entries until ItemEnd();
    ...
    for line in content.split('\n'):
        ...
        if 'ItemBegin' in line:
            # Match: ItemBegin("5217", "","UTDD. DUSHANBE");
        ...
        if 'ItemLink' in line and in_airport:
            # Match: ItemLink("../aiptjk/ad/ad2/utdd/ad2-utdd-031-031-1.pdf","(31) AERODROME CHART - ICAO");
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        print("Tajikistan AIP Scraper")
        print("Airports: UTDD (Dushanbe), UTDL (Khujand), UTDK (Kulob), UTDT (Bokhtar)")
        print("\nUsage: python tajikistan_scraper.py <ICAO> [--verbose]")
        print("Example: python tajikistan_scraper.py UTDD")
