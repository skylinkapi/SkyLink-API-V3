"""
Kyrgyzstan KAN Scraper
Fetches aerodrome charts from Kyrgyzstan's KAN eAIP system.
Based on http://kan.kg/ais/eaip/
"""
...
class KyrgyzstanScraper:
    """Scraper for Kyrgyzstan KAN aerodrome charts."""
    
    BASE_URL = "http://kan.kg"
    # URL pattern with AIRAC cycle date
    AIP_URL_PATTERN = "/ais/eaip/2025-12-25-AIRAC/html/eAIP/UC-AD-2.{icao}-en-GB.html"
    GRAPHICS_BASE = "/ais/eaip/2025-12-25-AIRAC/graphics/eAIP/"
    ...
    def get_charts(self, icao_code: str) -> List[Dict[str, str]]:
        """
        Fetch aerodrome charts for a Kyrgyzstan airport.
        
        Args:
            icao_code: ICAO code of the airport (e.g., 'UCFM', 'UCFO')
        ...
        try:
            if self.verbose:
                print(f"Fetching Kyrgyzstan eAIP for {icao_code}...")
...
if __name__ == '__main__':
    ...
    icao = sys.argv[1] if len(sys.argv) > 1 else 'UCFM'  # Manas International Airport
    
    scraper = KyrgyzstanScraper(verbose=True)
